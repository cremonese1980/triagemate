# Phase 9.2 — Durable Idempotency + Message Flow Validation

---

## 📊 # STATE MARKER — PHASE 9.2
...
**Phase Lineage:**
```
v0.8.1 (Kafka Hardening) → Phase 9.1 (PostgreSQL Runtime) → Phase 9.2 (Durable Idempotency)
```
...
- **Restart-safe** — duplicate processing after crashes
- **Cluster-safe** — no coordination across instances
- **Production-ready** — synchronized maps are a stopgap
...
- **Verified message → decision → DB flow**
...
| E2E message → decision → DB flow | Observability dashboards |
...
- **DB-enforced uniqueness** — no application-level race conditions
- **No race condition window** — constraint violation is atomic
- **Restart-safe** — survives application crashes
...
### Task 9.2.c — Service Integration
...
Check if eventId exists in DB
    ↓
If exists → skip processing (log idempotent hit)
    ↓
If not → persist event + emit decision (atomic transaction)
```
...
### Task 9.2.d — Remove In-Memory Idempotency
...
### Task 9.2.e — Deserialization Hardening

**Deliverable:** Enhanced error handling for malformed messages

Already using `ErrorHandlingDeserializer`. Now enforce:
- **Null payload detection** — reject before processing
- **Malformed JSON isolation** — log and skip without crash
- **Consumer resilience** — no listener container shutdown
...
#### Test 9.2.2 — Duplicate Event Detection
```java
@Test
...
#### Test 9.2.3 — Restart Persistence
```java
@Test
...
#### Test 9.2.4 — Malformed Payload Resilience
```java
@Test
...
#### Test 9.2.5 — Null Payload Handling
```java
@Test
...
- ✅ `processed_events` table exists and is auto-created in test profile
- ✅ `ProcessedEvent` JPA entity created
- ✅ `ProcessedEventRepository` created and injected
- ✅ `DatabaseIdempotencyGuard` implementation complete
- ✅ In-memory idempotency code removed (no `ConcurrentHashMap`, no `synchronized`)
- ✅ Duplicate detection works across application restarts
...
- ✅ CI pipeline green (no test context wiring failures)
- ✅ Deserialization hardening complete (null checks, error isolation)
- ✅ `application-test.yml` includes proper datasource configuration
...
- ✅ `phase9.md` updated with Phase 9.2 completion
...
**No new dependencies required** — Phase 9.1 infrastructure is sufficient.
...
### Issue 9.2.CI-1 — Test Profile Missing Datasource Configuration
...
1. **Zero in-memory state** — no `ConcurrentHashMap`, no `synchronized` blocks
2. **Restart resilience** — duplicate detection survives application crashes
3. **CI stability** — all integration tests green, no flakiness
4. **Clear error handling** — malformed messages logged, not crashed
5. **Database truth** — `processed_events` table is single source of idempotency truth

---

**Document Version:** 1.0  
**Last Updated:** 2026-02-23  
**Status:** Design phase complete — BLOCKED on test datasource configuration fix  
