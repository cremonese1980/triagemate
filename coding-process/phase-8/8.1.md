...
Completed:
- ErrorHandlingDeserializer configured (key + value)
- Malformed JSON does not crash consumer
- Null payload guard in listener
- Offset semantics verified
- DefaultErrorHandler with FixedBackOff
- DeadLetterPublishingRecoverer wired (.dlt topics)
- Retry/backoff configurable via TriagemateKafkaProperties
